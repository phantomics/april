    ...
    (if (or (complexp omega) (complexp alpha))
        (if (zerop (funcall (apl-residue comparison-tolerance)
            ...
            alpha (if (or (not (integerp (realpart omega)))
                      ...
                      (let* ((rlromega #+(or abcl ecl) (sb-rationalize (realpart omega))
                                       #+(not (or abcl ecl)) (rationalize (realpart omega)))
                             (rliomega #+(or abcl ecl) (sb-rationalize (imagpart omega))
                                       #+(not (or abcl ecl)) (rationalize (imagpart omega)))
                             (oden (lcm (denominator rlromega)
                                        (denominator rliomega)))
                             (rlralpha #+(or abcl ecl) (sb-rationalize (realpart alpha))
                                       #+(not (or abcl ecl)) (rationalize (realpart alpha)))
                             (rlialpha #+(or abcl ecl) (sb-rationalize (imagpart alpha))
                                       #+(not (or abcl ecl)) (rationalize (imagpart alpha)))
