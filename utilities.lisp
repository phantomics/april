...
(defmacro avector (&rest items)
  ...
  (let* ((first-item (first items))
	 (vector-type (cond ((typep first-item 'fixnum)
			     'fixnum)
			    ((typep first-item 'single-float)
			     'single-float)
			    (t t))))
    (loop :for item :in items :do (let ((item (if (listp item)
						  (if (eql 'avatom (first item))
						      (second item)
						      item)
						  item)))
				    (if (not (typep item vector-type))
					(setq vector-type t))))
    `(make-array (list ,(length items))
		 :element-type (quote ,vector-type)
...
(defun each-boolean (test omega &optional alpha)
  (let ((output (make-array (dims omega) :element-type 'bit)))
    (across omega (lambda (elem coords)
		    (if (if alpha (= 1 (funcall test elem (apply #'aref (cons alpha coords))))
			    (= 1 (funcall test elem)))
...
(defun each-scalar (function omega &optional alpha)
  (let ((output (make-array (dims omega)))
	(type))
    (macrolet ((conditional-type-assign (symbol &rest types)
		 `(setq ,symbol (cond ,@(append (loop :for type :in types
						   :collect `((typep result ,type) ,type))
						(list (list t t)))))))
      (across omega (lambda (elem coords)
		      (let ((result (if alpha (funcall function elem (apply #'aref (cons alpha coords)))
					(funcall function elem))))
			(if type (if (not (typep result type))
				     (setq type (cond ((eql 'bit type)
						       (conditional-type-assign type 'fixnum 'bignum))
						      ((eql 'fixnum type)
						       (conditional-type-assign type 'bignum))
						      ((eql 'single-float type)
						       (conditional-type-assign type 'double-float))
						      (t (setq type t)))))
			    (if (not (eq t type))
				(conditional-type-assign type 'bit 'character 'fixnum 'bignum
...
(defun apply-scalar (function alpha &optional omega &key (is-boolean nil))
  ...
  (macrolet ((for-each (&body body) `(funcall (if (and is-boolean (not (eq t (element-type alpha)))
						       (or (not omega)
