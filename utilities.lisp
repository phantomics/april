                 ...
                 (loop :for item :in form :for ix :from 0
                       :do (cond ((listp item)
                                  (if (and (second item) (not (third item))
                                      ...
                                      (let ((is-lexical
                                              (and inside-function (not (eql 'inwsd (first item)))
                                                   (not (char= #\* (aref (string (second item)) 0))))))
                                        (setf (nth ix form)
                                              (funcall (if (or is-lexical inside-function
                                                               (member (second item) *system-variables*)
...
(defmacro fn-ref (symbol)
  (let* ((space (package-name (symbol-package symbol)))
         (nspoint-sym (intern "*NS-POINT*" space)))
    `(if ,nspoint-sym (follow-path (append ,nspoint-sym (list ',symbol)))
         ,symbol)))

;; (defmacro fn-ref (symbol)
;;   (let* ((space (package-name (symbol-package symbol)))
;;          (nspoint-sym (intern "*NS-POINT*" space)))
;;     `(if ,nspoint-sym (follow-path (first ,nspoint-sym) (append (rest ,nspoint-sym) (list ',symbol)))
;;          ,symbol)))
...
(defmacro inv-fn (function &optional is-dyadic inverse-type)
  ...
  (let ((metadata (gensym)) (inverse (gensym))
        (function (if (not (and (listp function) (eql 'function (first function))
                      ...
                      function (list 'function (second (second function))))))
...
(defun follow-path (path &optional item)
  "Follow a path through a namespace and fetch the function or value there."
  (if (not path)
      item (if item (follow-path (rest path)
                                 (getf item (intern (string (first path)) "KEYWORD")))
               (if (boundp (first path))
                   (follow-path (rest path) (symbol-value (first path)))))))
...
(defun process-path (item key-list &optional processor value) ;; TODO: obsolete?
...
(defmacro a-set (symbol value &key (by) (axes) (fn-path-sym))
  ...
  (if (or (not (listp symbol))
          (eql 'fn-ref (first symbol)))
      (let* ((symbol (if (not (listp symbol))
                         symbol (second symbol)))
             (is-symbol-value (or (symbolp value)
                                  (and (listp value)
                                       (or (member (first value) '(inws inwsd fn-ref))
      ...
      (if (eql 'nspath (first symbol))
          ...
          (let* ((root-sym (if (not (and (listp (second symbol)) (eql 'fn-ref (caadr symbol))))
                               (second symbol) (second (second symbol))))
                 (is-symbol-value (or (symbolp value)
                                      (and (listp value)
                                           (or (member (first value) '(inws inwsd))
                                               ...
                                               (and (eql 'a-set (first value))
                                                    (or (symbolp (second value))
                                                        (and (listp (second value))
                                                             (member (second value) '(inws inwsd)))))))))
                 (ns-sym (intern "*NS-POINT*" (package-name (symbol-package root-sym))))
                 (ns-symb (if (boundp ns-sym) ns-sym))
            ...
            (if by
                ...
                `(if ,ns-symb (progn ,@(if fn-path-sym
                                     ...
                                     (set-path (first ,ns-symb)
                                               (append (loop :for i :in (rest ,ns-symb)
                                                             :collect (intern (string i) "KEYWORD"))
                                                       (list ,(intern (string root-sym) "KEYWORD"))
                                                       ',key-path)
...
(defmacro a-out (form &key (print-to) (output-printed)
  ...
  (let ((result (gensym)) (printout (gensym))
        ...
        (function-name-value (if (and (listp form) (eql 'function (first form)))
                                 (if (and (listp (second form)) (eql 'fn-ref (first (second  form))))
                                     `(string (quote ,(second (second form))))
        ...
        (form (if (and (characterp form) (of-lexicons this-idiom form :functions))
                  ...
                  (if (not (and (listp form) (eql 'function (first form))
                      ...
                      form (list 'function (second (second form)))))))
    `(let* ((,result ,form)
    ...
    (let ((is-scalar (or (and (second arguments)
      ...
      (or (join-fns `(a-call ,function ,@arguments))
          (if (and (listp function)
              ...
              ;; special case to handle ⎕CS, since it needs the workspace's *ns-point* symbol
              (let* ((arg (first arguments))
                     (workspace (if (symbolp arg)
                                     ...
                                     (cons 'nspath
                                           (append (if (listp namespace) namespace
                                                   ...
                                                   (list (intern (if (listp (second function))
                                                                 ...
                                                                 "KEYWORD"))
                                                   (cddr function))))))
                         (if (eql 'fn-ref (first function))
                             (setq function (second function))
                             (if (and (eql 'function (first function))
                                      (listp (second function)) (eql 'fn-ref (first (second function))))
                                 (let* ((symbol (second (second function)))
                                        (sym-package (package-name (symbol-package symbol)))
                                        (ns-sym (intern "*NS-POINT*" sym-package)))
                                   (setq function `(if ,ns-sym
                                                       (symbol-value
                                                        (intern (format-nspath
                                                                 (append ,ns-sym
                                                                         (list ,(intern (string symbol)
                                                                                        "KEYWORD"))))
                                                                ,sym-package))
                                                       (function ,symbol))))))))
...
(defmacro a-comp (symbol &rest body)
  ...
  (declare (ignore symbol))
  (let ((expanded (macroexpand (loop :for i :in body :collect (if (and (listp i) (eql 'function (first i))
                                                                       (listp (second i))
                                                                       (eql 'fn-ref (first (second i))))
                                                                  `(function ,(second (second i)))
                                                                  (if (and (listp i) (eql 'fn-ref (first i)))
                                                                      (second i)
                                                                      i))))))
