...
(defun disclose-atom (item)
  ...
  (if (and (not (stringp item))
      ...
      (apply #'aref (cons item (loop :for i :from 0 :to (1- (rank item)) :collect 0)))
      item))
...
(defun apply-scalar (function alpha &optional omega)
  ...
  (if (not omega)
	...
	(flet ((disclose-non-vector (input)
		 (if (not (and (is-unitary input) (arrayp (aref input 0))
			       (< 1 (rank (aref input 0)))))
		     input (disclose input))))
	  ;; (print (list :ao alpha omega))
	...
	body (enclose-axes
	      `(aref-eliding ,body (mapcar (lambda (array)
					     (if array (aops:each (lambda (elem)
								    (apply-scalar #'- elem index-origin))
								  array)))
					   (list ,@axes))
			     ,@(if set (list :set set)))
	      (rest axis-sets)))))
...
(defun over-operator-template (axes function &key (first-axis nil) (for-vector nil) (for-array nil))
  ...
  `(lambda (omega)
	...
	`(let ((new-array (copy-array omega)))
	   ...
	   (funcall (lambda (item) ;; (if (or (not (arrayp item))
				   ;; 	   (not (= 1 (rank item)))
				   ;; 	   (not (arrayp (aref item 0)))
				   ;; 	   (and (= 1 (rank (aref item 0)))
				   ;; 		(is-unitary (aref item 0))))
				   ;;     item (vector item))
			    (if (= 1 (array-depth omega))
				item (vector item)))
