...
;;   ;; in the workspace before compilation is performed so that recursive
;;   ;; functions will work correctly as with fn←{A←⍵-1 ⋄ $[A≥0;A,fn A;0]} ⋄ fn 5
;;   ;; (print (list :tok tokens closure-meta))
;;   (match tokens
;;     ((list (guard fn-form (and (listp fn-form)
;;                                (eq :fn (first fn-form))
;;                                (listp (second fn-form))))
...
;;                   (if (not (fboundp (intern (string symbol) space)))
;;                       (setf (symbol-function (intern (string symbol) space)) #'dummy-nargument-function))))
;;        (setq tokens (list (list (first fn-form) (second fn-form)
;;                                 (lexer-postprocess (third fn-form) idiom space fn-meta))
;;                           '(:fn #\←) symbol))
;;        ))
...
;;        (if (and form-meta form-content)
;;            (setq tokens (list (first fn-form) (second fn-form)
...
;;      (let ((fn-symbol (intern (string symbol) space))
;;            (op-meta (second op-form))
;;            (int-symbol (intern (concatenate 'string (if (eq :lateral (getf (second op-form) :valence))
;;                                                         "𝕆𝕃∇" "𝕆ℙ∇")
;;                                             (string symbol))
;;                                space)))
;;        ;; (print (list :op (getf op-meta :valence)))
...
;;        (setq tokens (list (list (first op-form) (second op-form)
...
;;        ;; (print (list :op (getf op-meta :valence)))
;;        ;; if the symbol is already bound as a regular function, unbind it
;;        (setq tokens (list (cons :axes (loop :for item :in each-axis
;;                                          :collect (lexer-postprocess item idiom space closure-meta)))
;;                           axes-of))
;;        ;; (print (list :opt tokens each-axis))
;;        ))
;;     ((guard list (and (listp list) (not (member (first list) '(inws inwsd)))))
;;      (setq tokens (loop :for item :in list :collect (lexer-postprocess item idiom space closure-meta)))
;;      ;; (print (list :lout tokens))
;;      )
;;     ((guard symbol (eql symbol '∇∇))
;;      ;; (print (list :sy symbol))
;;      (setq tokens (list :op (getf closure-meta :valence) '∇∇)))
;;     (token-or-tokens (setq tokens token-or-tokens))
;;     )
;;   ;; (assigner tokens (processor))
;;   ;; (print (list :tok tokens))
...
;;                                            :pivotal :lateral)))
;;               (int-symbol (if is-operator (intern (concatenate 'string (if (eq valence :lateral) "𝕆𝕃∇" "𝕆ℙ∇")
;;                                                                (string symbol))
;;                                                   space))))
    ...
    (match tokens
      ((list (guard fn-form (and (listp fn-form)
       ...
       (let* ((fn-meta (second fn-form))
              ...
              (int-symbol (if is-operator (intern (concatenate 'string (if (eq valence :lateral) "𝕆𝕃∇" "𝕆ℙ∇")
                                                               (string symbol))
                                                  space))))
         ;; (print (list :sym symbol fn-form closure-meta-form))
         (if is-operator (progn (setf (getf (rest fn-meta) :valence) valence)
                                (if (getf (rest fn-meta) :valence-setters)
                                    (loop :for setter :in (getf (rest fn-meta) :valence-setters)
                                       :do (funcall setter valence)))
      ...
      ((guard fn-form (and (listp fn-form) (eq :fn (first fn-form))))
       (let* ((form-meta (if (listp (second fn-form)) (second fn-form)))
         ...
         ;; (print (list :ab form-meta fn-form))
         ;; (print (list :frr form-meta processed is-operator))
         (if (not (and form-meta form-content))
             tokens (list (if is-operator :op :fn)
                          (second fn-form)
       ...
       (labels ((process-symbols (possible-symbols &optional top-level)
         ...
         (let* ((assignment-indices)
                (items (loop :for item :in list :for ix :from 0
                          ...
                          :collect ;;(if assignment-index item
                            (lexer-postprocess item idiom space closure-meta-form)))
           ...
           ;; (if assignment-index
           ;;     (loop :for symbol :in (alexandria:flatten (nthcdr (1+ assignment-index) list))
           ;;        :do (if (and (symbolp symbol) (not (keywordp symbol)))
           ;;                (if closure-meta (if (and (not (member symbol (getf closure-meta :var-syms)))
           ;;                                          (not (member symbol '(⍺ ⍵ ⍶ ⍹ ⍺⍺ ⍵⍵ ∇ ∇∇))))
           ;;                                     (push symbol (getf closure-meta :var-syms)))
           ;;                    ;; TODO: URGENT - bla ← {⍺×⍵+3} ⋄ x bla←5 won't work with this.
           ;;                    ;; or maybe not? since no function will be found to the right of the ←
           ;;                    ;; also need to handle (:fn ...) to the left of ←
           ;;                    (progn (if (fboundp symbol) (fmakunbound symbol))
           ;;                           (set symbol nil)))
           ;;                )))
           ;; (print (list :asn assignment-index (if assignment-index (nthcdr (1+ assignment-index) list))))
           items)))
      ((guard symbol (eql symbol '∇∇))
       (let ((this-form (list :op :valence '∇∇)))
         (push (lambda (valence) (setf (second this-form) valence))
