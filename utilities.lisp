...
(defmacro nspath (list &rest keys)
  ...
  (labels ((process-key (item key-list)
             (if (not key-list)
                 item (process-key `(getf ,item ,(intern (string (first key-list))
                                                         "KEYWORD"))
                                   (rest key-list)))))
    (process-key list keys)))

(defmacro a-set (symbol value)
                ...
                (labels ((follow-path (item path)
                           (if (not path)
                               item (follow-path `(getf ,item ,(first path))
                                                 (rest path)))))
                  (if namespace (if (eq nil (first namespace))
                                    (if (symbolp (rest namespace))
                                        `(setf ,(rest namespace)
                                               ',(list (intern (string symbol) "KEYWORD")
                                                       set-to)
                                               ,ns-sym ,(rest namespace))
                                        `(setf ,(follow-path (rest namespace))
                                               ,set-to))
                                    (print `(setf (getf ,ns-sym ,(intern (string symbol) "KEYWORD"))
                                           ,set-to)))
                      `(setf ,symbol ,set-to))))))
