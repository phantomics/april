...
;;     (assign-axes function-axes process item items rest-items)
...
;; 	      ;; (symbol-axes (getf symbol-props :axes))
;; 	      )
;; 	  (print (list :fn precedent fn-content fn-element fn-sym asop symbol))
;; 	  (print (list :it items))
...
(defun operator-assignment (tokens space process &optional precedent properties pre-properties)
  ...
  (let* ((asop) (asop-props) (symbol) (symbol-props)
	...
	(assign-element asop asop-props process-function process '(:glyph ←) space item items rest-items))
    ;; (if asop (list :oo (print :cc)))
    (if asop (assign-element symbol symbol-props process-value process '(:symbol-overriding t)
			     space item items rest-items))
    ;; (print (list :it items rest-items symbol))
    ;; (if (not symbol) (setq items rest-items item (first items) rest-items (rest items)))
    ;; (if symbol (error "test"))
    (let* ((operator-symbol (intern (concatenate 'string (if (member :pivotal preceding-type)
							     "𝕆ℙ∇" "𝕆𝕃∇")
						 (string symbol))))
...
;;       (progn (assign-axes function-axes process item items rest-items)
;; 	     (assign-subprocessed fn-element function-props process
;; 				  (list :special '(:omit (:value-assignment :function-assignment)))
;; 				  item items rest-items)
;; 	     (setq is-function (eq :function (first (getf function-props :type))))
;; 	     (print (list :eo is-function items fn-element function-props))
;; 	     (if is-function
...
;; 		 			    :valence :lateral)
;; 		 		      item items rest-items))
;; 	     (print (list :ff fn-element value items))
