...
;; (defun array-inner-product (omega alpha function1 function2)
;;   "Find the inner product of two arrays with two functions."
;;   (let* ((odims (butlast (dims omega))) (adims (rest (dims alpha)))
;; 	 (osegment (reduce #'* odims)) (asegment (reduce #'* adims))
;; 	 (odisp (make-array osegment :displaced-to omega :element-type (element-type omega)))
;; 	 (aholder (make-array asegment :element-type (element-type alpha)))
;; 	 (output (make-array (append adims odims))))
;;     (across output (lambda (elem coords)
;; 		     (let ((result)
;; 			   (ostart (reduce #'* (butlast coords)))
;; 			   (astart (reduce #'* (rest coords))))
;; 		       (adjust-array odisp osegment :displaced-to omega
;; 				     :displaced-index-offset (* osegment ostart))
;; 		       (loop :for i :below asegment
;; 			  :do (setf (aref aholder i) (row-major-aref alpha (+ astart (* i asegment)))))
;; 		       (loop :for x :across (april::apply-scalar function1 odisp aholder)
;; 			  :do (setq result (if (not result) x (funcall function2 x result))))
;; 		       (setf (apply #'aref output coords) result))))
;;     output))
