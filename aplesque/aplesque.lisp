...
;;   "Find the inner product of two arrays with two functions."
;;   (let* ((adims (butlast (dims alpha)))
;; 	 (odims (or (rest (dims omega)) (dims omega)))
;; 	 (asize (size alpha)) (osize (size omega))
;; 	 (osegment (first (dims omega))) (asegment (first (last (dims alpha))))
;; 	 (adisp (make-array asegment :displaced-to alpha :element-type (element-type alpha)))
;; 	 (oholder (if (= osegment (size omega))
;; 		      omega (make-array asegment :element-type (element-type alpha))))
;; 	 (output (make-array (print (loop :for x :in (append (rest (dims alpha)) (butlast (dims omega)))
;; 				       :when (< 1 x) :collect x)))))
;;     ;; (print (list :x adims odims asegment osegment output))
;;     (loop :for x :below (size output)
;;        :do (let ((result)
;; 		 (aoffset (* asegment (mod (floor (/ x (max 1 (/ (/ asize asegment)
;; 								 (/ osize asegment)))))
;; 					   (/ asize asegment))))
;; 		 (ostart (mod (floor (/ x (max 1 (/ (/ osize asegment)
;; 						    (/ asize  asegment)))))
;; 			      (/ osize asegment))))
;; 	     (adjust-array adisp asegment :displaced-to alpha :displaced-index-offset aoffset)
;; 	     (loop :for i :below osegment
;;      		:do (setf (aref oholder i) (row-major-aref omega (+ ostart (* i osegment)))))
;; 	     (print (list :aa aoffset ostart adisp oholder))
;; 	     (loop :for x :across (april::apply-scalar function1 oholder adisp)
