...
(defun across (array function &key (start-at nil) (limit nil) (indices nil))
  ...
  (let* ((dimensions (dims array))
	 (this-start (first start-at))
	 (this-limit (first limit)))
    (loop :for elix :from (if this-start this-start 0)
       :to (min (if this-limit (+ -1 this-limit (if this-start this-start 0))
		    (1- (nth (length indices) dimensions)))
		(1- (nth (length indices) dimensions)))
       :do (if (< (length indices) (1- (length dimensions)))
	       (across array function :start-at (rest start-at) :limit (rest limit)
		       :indices (append indices (list elix)))
  ...
  (flet ((sort-dimensions (outer inner)
    ...
    (let* ((arrays-disp (make-array (list (array-total-size arrays)) :displaced-to arrays))
	   ...
	   (output (make-array (sort-dimensions (dims arrays)
						(loop :for dim :from 0 :to (1- (length (first each-dims)))
						   :collect (reduce #'max (mapcar (lambda (d) (nth dim d))
										  each-dims))))
			       :initial-element 0)))
      ;; (print (list each-dims axis output))
			       ...
			       (cond ((arrayp elem)
					...
					(destructuring-bind (ren-width &optional (ren-height 1))
					  ...
					  (setf (apply #'aref (cons strings coords))
						rendered)
					  (if (> (+ 1 ren-width (min 1 last-coord))
					  ...
					  (if (> ren-height (aref y-offsets (1+ row)))
					      (setf (aref y-offsets (1+ row))
						    (+ (if (= 0 (aref y-offsets (1+ row)))
							   0 -1)
						       ren-height (aref y-offsets (1+ row))))))))
				     ((numberp elem)
					...
					(setf (apply #'aref (cons strings coords))
					      elem-string)
					(if (> (+ 1 elem-length (min 1 last-coord))
					       (- (aref x-offsets (1+ last-coord))
						  (aref x-offsets last-coord)))
					    (setf (aref x-offsets (1+ last-coord))
						  (+ elem-length (min 1 last-coord)
						     (if (not (and prior-elem (arrayp prior-elem)
								   ...
								   (let ((first-prior (row-major-aref prior-elem 0)))
				     ...
				     ((characterp elem)
				      ...
				      (setf (apply #'aref (cons strings coords))
					    ...
					    (max 1 (+ (min 1 last-coord)
						      (aref x-offsets last-coord))))))
			       ;; (print (list :yy y-offsets last-coord))
			       (if (= 0 last-coord)
...
(defmacro matrix-print (array &rest options)
  (let ((rendered (gensym)))
    `(let ((,rendered (matrix-render ,array ,@options)))
       (if (stringp ,rendered)
	   ,rendered
	   (make-array (list (array-total-size ,rendered))
