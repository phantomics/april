	 ...
	 (join (array1 array2)
	   (let ((output (make-array (loop :for dim :in (dims array1) :counting dim :into dx
					:collect (if (/= axis (1- dx))
				     ...
				     :element-type (cond ((eql (element-type array1)
							       (element-type array2))
							  (element-type array1))
							 ((and (eql 'fixnum (element-type array1))
							       (eql 'bit (element-type array2)))
							  'fixnum)
							 ((and (eql 'bit (element-type array1))
							       (eql 'fixnum (element-type array2)))
							  'fixnum)
							 ((and (eql 'bignum (element-type array1))
							       (eql 'bit (element-type array2)))
							  'bignum)
							 ((and (eql 'bit (element-type array1))
							       (eql 'bignum (element-type array2)))
							  'bignum)
							 ((and (eql 'fixnum (element-type array1))
							       (eql 'bignum (element-type array2)))
							  'bignum)
							 ((and (eql 'bignum (element-type array1))
							       (eql 'fixnum (element-type array2)))
							  'bignum)
							 ((and (eql 'short-float (element-type array1))
							       (eql 'long-float (element-type array2)))
							  'long-float)
							 ((and (eql 'long-float (element-type array1))
							       (eql 'short-float (element-type array2)))
							  'long-float)
							 (t t)))))
	     ...
	     (symbol-macrolet ((this-string (apply #'aref (cons strings coords)))
	       ...
	       (across input (lambda (elem coords)
			       (let* ((last-coord (first (last coords)))
				      ...
				      (rendered (apply #'aref (cons strings coords))))
