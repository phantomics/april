...
(defun aref-eliding (array indices &key (adims (dims array)) (path nil) (fn #'identity) (set nil))
  (macrolet ((refer-to (value-form)
	       `(if (rest indices)
		    (aref-eliding array (rest indices)
				  :set set :fn fn :adims adims :path (cons ,value-form path))
		    (funcall fn (if set (setf (apply #'aref (cons array (reverse (cons ,value-form path))))
					      set)
				    (apply #'aref (cons array (reverse (cons ,value-form path)))))))))
    ;; (if set (setq fn (lambda (i) (declare (ignore i)) (setf i set))))
    ;; (print (list :ii indices array))
    (if (not path)
	(if (< (length indices)
	       (length adims))
