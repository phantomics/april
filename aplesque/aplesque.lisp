...
(defun run-dim (array function &key (dimensions nil) (indices nil) (start-at nil) (limit nil) (elision nil))
  ...
  (let ((dimensions (if dimensions dimensions (dims array))))
    (flet ((for-element (elix)
      ...
      ;; (print (list :in indices dimensions elision (if indices (nth (1- (print (length indices)))
      ;; 								   elision))))
      ;; (print (list :ll (length indices) (nth (length indices) elision)))
      (if (and elision (nth (length indices)
			    elision))
	  (for-element (nth (length indices)
			    elision))
	  (loop for elix from (if start-at (nth (length indices) start-at)
				  0)
	     to (min (if limit
			 (+ (if start-at (nth (length indices) start-at)
				0)
...
(defun aref-eliding (array &rest indices)
  (if (and (= (length indices)
      ...
      (let* ((adims (dims array))
	     ...
	     (sub-dims (loop for index from 0 to (1- (length indices))
			  when (not (nth index indices))
			  do (setq el-indices (cons index el-indices))
			  when (not (nth index indices))
			  collect (nth index adims)))
