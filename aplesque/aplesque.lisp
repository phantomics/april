    ...
    (let* ((a1 (if (not (is-unitary a1))
	  ...
	  (let* ((lesser-first (< (rank a1) (rank a2)))
	    ...
	    (if (and (= 1 (- grank lrank))
			 ...
			 (let ((compare-dims (loop :for ix :from 0 :to (1- grank)
...
(defun run-dim (array function &key (start-at nil) (limit nil) (indices nil))
  ...
  (let* ((dimensions (dims array))
	 ...
	 (this-limit (nth (length indices) limit)))
    (loop :for elix :from (if this-start this-start 0)
       :to (min (if this-limit (+ (if this-start this-start 0)
				  this-limit)
...
(defun matrix-render (array)
  (labels ((measure-height (dims &optional index count)
	     (let ((count (if count count 0))
		   (index (if index index 0)))
	       (if (not dims)
		   count (measure-height (rest dims) (1+ index)
					 (+ count (apply #'* (cons (- (first dims)
								      (min 1 index))
								   (cons (max 1 index)
									 (rest dims)))))))))
	   ;; (y-offset (coords dims)
	   ;;   ;; (print (list :cc coords dims))
	   ;;   (apply #'+ (mapcar #'+ (mapcar #'* coords (let ((current 1))
	   ;; 						 (loop :for dim :in (cons 1 dims)
	   ;; 						    :collect (setq current (* current dim)))))
	   ;; 			;; '(0 0 1 7)
	   ;; 			;; (print (mapcar #'* coords (cons 0 (iota (length dims) :start 0))))
	   ;; 			(mapcar #'* coords (cons 0 (iota (length dims) :start 0)))
	   ;; 			(mapcar #'* coords (cons 0 (cons 1 (rest dims))))
	   ;; 			)))
	   )
    (let* ((adims (dims array))
	   (widths (make-array (list (first (last adims))) :initial-element 1))
	   (x-offsets (make-array (list (first (last adims))) :initial-element 0))
	   (y-offsets (make-array (list (reduce #'* (rest (reverse adims)))) :initial-element 0))
	   (strings (make-array adims))
	   (y-intervals (measure-height adims)))
      (run-dim array (lambda (elem coords)
		       (print (list :cc (rest (reverse coords))
		       		    (rest (reverse (rest adims)))
		       		    (reduce #'+ (mapcar #'* (rest (reverse coords))
		       					(let ((current 1))
		       				 (loop :for dim :in (cons 1 (rest (reverse (rest adims))))
						    :collect (setq current (* current dim))))))))
		       (let ((row (reduce #'+ (mapcar #'* (rest (reverse coords))
						      (let ((current 1))
							(loop :for dim :in (cons 1 (rest (reverse (rest adims))))
							   :collect (setq current (* current dim)))))))
			     (stack (cons 1 (let ((current 0))
					      (loop :for dim :in (rest (reverse (rest adims)))
						 :collect (setq current (+ current dim)))))))
			 (print (list :st elem stack (mapcar #'* (rest (reverse coords))
							stack)
				      (reduce #'+ (mapcar #'* (rest (reverse coords))
							  stack))))
			 (if (< row (1- (length y-offsets)))
			     (setf (aref y-offsets (1+ row))
				   (1+ row))))
		       ;; (print (first (last coords)))
		       (if (numberp elem)
			   (let ((elem-string (write-to-string elem)))
			     (setf (apply #'aref (cons strings coords))
				   elem-string)
			     (if (> (+ (min 1 (first (last coords)))
				       (length elem-string))
				    (aref widths (first (last coords))))
				 (setf (aref widths (first (last coords)))
				       (+ (min 1 (first (last coords)))
					  (length elem-string)))))
			   (if (characterp elem)
			       (setf (apply #'aref (cons strings coords))
				     elem)))))
      (print y-offsets)
      (let ((output (make-array (list (measure-height (rest (reverse adims)))
				      (1+ (loop :for wix :from 0 :to (1- (length widths))
					     :summing (aref widths wix)
					     :into height
					     :do (if (/= wix (1- (length widths)))
						     (setf (aref x-offsets (1+ wix))
							   height))
					     :finally (return height))))
				:element-type 'character :initial-element #\ )))
	;; (print (list :wd widths x-offsets (measure-height (print (cdr (reverse adims))))))
	(run-dim strings (lambda (string coords)
			   (let ((row (reduce #'+ (mapcar #'* (rest (reverse coords))
							  (let ((current 1))
							    (loop :for dim :in (cons 1 (rest (reverse (rest adims))))
							       :collect (setq current (* current dim))))))))
			     (if (stringp string)
				 (loop :for strix :from 0 :to (1- (length string))
				    :do (setf (aref output ;; (y-offset (rest (reverse coords))
							   ;; 	     (rest (reverse (rest adims))))
						    (aref y-offsets row)
						    (+ 1 strix (aref x-offsets (first (last coords)))
						       (- (aref widths (first (last coords)))
							  (length string))))
					      (aref string strix)))
				 (setf (aref output ;; (y-offset (rest (reverse coords))
						    ;; 	      (rest (reverse (rest adims))))
					     (aref y-offsets row)
					     (1+ (aref x-offsets (first (last coords)))))
				       string)))))
	(loop :for row :from 0 :to (1- (first (dims output)))
	   :do (setf (aref output row 0) #\Newline))
	(print (dims output))
	output))))

(defun matrix-print (array)
  (let ((rendered (matrix-render array)))
    (make-array (list (array-total-size rendered)) :element-type 'character :displaced-to rendered)))
