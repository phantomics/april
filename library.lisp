...
(defun match-lexical-function-identity (glyph)
  (second (assoc (aref (string glyph) 0)
		 '((#\+ 0) (#\- 0) (#\× 1) (#\÷ 1) (#\⋆ 1) (#\* 1) (#\! 1) 
		   (#\< 0) (#\≤ 1) (#\= 1) (#\≥ 1) (#\> 0) (#\≠ 0) (#\| 0)
...
(defun operate-reducing (function axis last-axis empty-array-identity)
  (lambda (omega)
    (if (= 0 (size omega))
	(if (= 1 (rank omega))
  ...
  (lambda (omega &optional alpha)
    (let* ((odims (dims omega)) (adims (dims alpha))
	   (odivs (if (< rank (rank omega))
		      (make-array (subseq odims 0 (- (rank omega) rank)))))
	   (odiv-dims (subseq odims (- (rank omega) rank)))
	   (odiv-size (reduce #'* odiv-dims))
	   (adivs (if (and alpha (< rank (rank alpha)))
		      (make-array (subseq adims 0 rank))))
	   (adiv-dims (if adivs (subseq adivs (- (rank alpha) rank))))
	   (adiv-size (if alpha (reduce #'* adiv-dims)))
	   (output)) ;; the output variable is set if neither array needs to be subdivided
      (flet ((generate-divs (div-array ref-array div-dims div-size)
	       (dotimes (i (size odivs))
	...
	(if adivs (generate-divs adivs alpha adiv-dims adiv-size))
	(if alpha (let ((both-matching-shape
			 (and (= (rank adivs) (rank odivs))
			      (loop :for o :in (dims odims) :for a :in (dims adivs) :always (= o a)))))
		    (if odivs (or (and adivs (not both-matching-shape) (error "Mismatched array shapes."))
				  (dotimes (i (size odivs))
				    (setf (row-major-aref odivs i)
					  (funcall function-dyadic (row-major-aref odivs i)
						   (if (not both-matching-shape)
						       alpha (row-major-aref adivs i))))))
			(if adivs (dotimes (i (size adivs))
				    (setf (row-major-aref adivs i)
					  (funcall function-dyadic omega (row-major-aref adivs i))))
			    (setq output (funcall function-dyadic omega alpha)))))
	    (if odivs (dotimes (i (size odivs))
			(setf (row-major-aref odivs i)
			      (funcall function-monadic (row-major-aref odivs i))))
