    ...
    (flet ((rebase (base-coords number)
	     (let ((operand number) (last-base 1)
		  ...
		  :do (setq last-base base
	       ...
	       element)))
      (let ((output (make-array (append (loop :for dim :in adims :when (< 1 dim) :collect dim)
	...
	(across alpha (lambda (aelem acoords)
			...
			(let ((ac (loop :for dx :below (length acoords) :when (< 1 (nth dx adims))
			  ...
			  (across omega (lambda (oelem ocoords)
					  (let ((out-coords (append ac (loop :for dx :below (length ocoords)
									  :when (< 1 (nth dx odims))
...
;; 	     (let ((base 1) (result 0))
;; 	       (loop :for index :from (1- last-adim) :downto 0
;; 		  :do
;; 		  (print (list :cc (1- last-adim)
;; 			       (cons index (rest number-coords))
;; 			       (append (butlast base-coords 1)
;; 				       (list index))))
;; 		  (incf result (* base (apply #'aref omega (cons index (rest number-coords)))))
;; 		  (setq base (* base (apply #'aref alpha (append (butlast base-coords 1)
;; 								 (list index))))))
;; 	       (print (list :end result))
;; 	       result)))
;;       (print output)
;;       (across alpha (lambda (aelem acoords)
;; 		      (declare (ignore aelem))
...
;; 				      (print (list :ao acoords ocoords))
;; 				      (rebase acoords ocoords)
;; 				      (setf (apply #'aref output ocoords)
;; 					    (rebase acoords ocoords))
;; 				      )
;; 			      :elements (loop :for i :below (rank omega) :collect (if ;(= i (1- (rank omega)))
;; 										   (= i 0)
;; 										   0)))))
;;       :elements (loop :for i :below (rank alpha) :collect (if (= i (1- (rank alpha)))
;; 							      0)))
;;     output))
  ...
  (flet ((rebase (bases numerators)
    ...
    (let* ((d-o (dims omega))
      ...
      (if (and (not (is-unitary alpha))
	       ...
	       (not (= (first d-a) (first d-o))))
	  (error (concatenate 'string "If neither argument to ⊥ is scalar, the first dimension of the left"
			      " argument must equal the first dimension of the right argument."))
					...
					(if numerators
					    (loop :for dm :below (first d-o)
					       :do (setf (aref numerators dm)
							 (apply #'aref omega (append (list dm)
