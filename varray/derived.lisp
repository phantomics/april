  ...
  (get-promised
   ...
   (let* ((out-shape (shape-of varray))
     ...
     (cond
       ...
       (t (case (getf params :base-format)
            (:encoded)
            (:linear)
            (t (lambda (index)
                 (let ((result) (subarrays) (sub-flag))
     ...
     (typecase base
       (vapri-integer-progression
        (case (getf params :base-format)
          (:encoded)
          (:linear)
          (t (lambda (index) (declare (ignore index)) 1))))
       ...
       (vader-where
        ...
        (case (getf params :base-format)
          (:encoded)
          (:linear)
          (t (lambda (index) (row-major-aref (vader-content base) index)))))
       (array
        (setf (vader-content varray)
              (make-array (shape-of varray) :element-type 'bit :initial-element 0))
    ...
    (if (zerop output-size)
        (let ((prototype (prototype-of varray)))
          (case (getf params :base-format)
            (:encoded)
            (:linear)
            (t (lambda (index) (declare (ignore index)) prototype))))
        (case (getf params :base-format)
          (:encoded)
          (:linear)
          (t (if (zerop (size-of (vader-base varray)))
                 (prototype-of (vader-base varray))
  ...
  (get-promised
   ...
   (let* ((assigning (getf params :for-selective-assign))
     ...
     (let* ((indexer (indexer-section (shape-of (vader-base varray))
            ...
            (prototype (unless assigning
                         (setf (varray-prototype varray)
                               (if (or (zerop size) (zerop base-size))
                                   (prototype-of (vader-base varray))
                                   (let ((indexed (funcall (second indexer) 0)))
                                     (if indexed
                                         (if (not (functionp base-indexer))
       ...
       (if assigning (list nil (lambda (index)
                                 (declare (type integer index))
                                 (let ((indexed (funcall (second indexer) index)))
                                   (if (not layers-below) indexed
                                       ...
                                       (let ((inext (funcall base-indexer index)))
                                         (when inext (if (eq :pick layers-below)
                                                         inext (funcall (second indexer)
                                                                        inext))))))))
           (if (functionp base-indexer) ;; TODO: why is the disclose needed?
               indexer (list nil (lambda (index)
                                   (when (funcall (second indexer) index)
    ...
    (if (getf params :for-selective-assign)
        ...
        (if (not (loop :for item :across (vasec-pad varray) :always (zerop item)))
            (let* ((composite-indexer (or (join-indexers indexers t) #'identity))
              ...
              (lambda (index) 
                (let ((indexed (funcall (second indexer) (funcall composite-indexer index))))
    ...
    (if (getf params :for-selective-assign)
        ...
        (if (vadex-separating varray)
            ...
            (case (getf params :base-format)
              (:encoded)
              (:linear)
              (t (generator-of (vader-base varray)
