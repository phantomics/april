...
;; (defmethod indexer-of ((varray vader-subvector) &optional params)
;;   (get-promised (varray-indexer varray)
;;                 (let* ((valix 0)
  ...
  (get-promised
   ...
   (let* ((indices)
     ...
     (flet ((verify-vindex (ind vector-index)
       ...
       ;; (⊂4 5 2 6 3 7 1)⌷'MARANGA'  st←'aodjeignwug' ⋄ st[⍋st]  
       ...
       (if (not set)
               ...
               (loop :for in :in indices :for ifactor :across ifactors
                     ...
                     :do (if (numberp in)
                             ...
                             (if in (let ((matched-index) (sub-index 0) (aindex index))
                                 ...
                                 (multiple-value-bind (index remainder)
                                     (floor remaining (aref ofactors ofix))
                                   (let ((indexed (when in (funcall (indexer-of in) index))))
           ...
           (if (vasel-assign-if varray)
               (let* ((assign-indexer (indexer-of (vasel-assign varray)))
                      (mask (funcall (vasel-assign-if varray) (vader-base varray)))
                      (mask-indexer (indexer-of mask)))
                 (if (or adims (eq :monadic assign-indexer)) ;; TODO: optimize types for mask indices
                     ;; the case of an array assigned depending on logical function result,
                     ;; for example 10 11 12 13@(<∘5)⍳9
                     (let ((mindices (make-array (shape-of (vader-base varray))
                       ...
                       (if (eq :monadic assign-indexer)
                           (dotimes (i (size-of mask))
                             (when (not (zerop (funcall mask-indexer i)))
                               (setf (row-major-aref mindices i)
                                     (1+ i))))
                           (dotimes (i (size-of mask))
                             (when (not (zerop (funcall mask-indexer i)))
                               (setf (row-major-aref mindices i) (incf mindex)))))
                       ;; (print (list :mm mindices))
                       (if (eq :monadic assign-indexer)
                           (let ((mindices (render (funcall (vasel-calling varray) mindices))))
                             (lambda (index)
                               (let ((this-mindex (row-major-aref mindices index)))
                                 (if (zerop this-mindex) (funcall base-indexer index)
                                     (funcall base-indexer (1- this-mindex))))))
                           (lambda (index) 
                             ;; the case of 10 11 12@(2∘|)⍳5
                             (let ((this-mindex (row-major-aref mindices index)))
                               (if (zerop this-mindex) (funcall base-indexer index)
                                   (funcall assign-indexer (1- this-mindex)))))))
                                 ...
                                 (if in (let ((matched-index) (sub-index 0) (aindex index))
                                          ...
                                          (if (or (and (vectorp in) (< 0 (length in)))
                                              ...
                                              (multiple-value-bind (index remainder)
                                                ...
                                                (let* ((sub-indexer (indexer-of in))
                                                       (sub-indexed (funcall sub-indexer 0))
                                                       (ssindexer (indexer-of sub-indexed))
  ...
  (get-promised
   ...
   (let* ((axis (vads-axis varray))
          ...
          (base-indexer (if (and (not (varrayp base-indexer))
                            ...
                            base-indexer (indexer-of base-indexer)))
     ...
     (cond
       ...
       ;;                                            ))
       ;;          :do (setf indexed (funcall (indexer-of indexed) 0)))
...
(defmethod prototype-of ((varray vader-subarray-split))
  ...
  (get-promised (varray-prototype varray)
                (let ((first-item (funcall (indexer-of varray) 0)))
                  ...
                  (let ((indexer (if (not (functionp base-indexer))
                    ...
                    (when (and (shape-of base) (not (shape-of indexer))
                               (or (arrayp indexer)
                                   (varrayp indexer)))
                      (setf (vads-subrendering varray) t))
                    ;; (print (list :in indexer base base-indexer (indexer-of indexer)))
                    indexer))))))
...
(defmethod indexer-of ((varray vader-pick) &optional params)
  ...
  (if (vapick-assign varray)
      ...
      (let* ((this-reference (fetch-reference varray (vader-base varray)))
        ...
        ;;     this-indexer (row-major-aref this-indexer 0))
...
(defmethod indexer-of ((varray vader-permute) &optional params)
  ...
  (get-promised
   ...
   (let* ((assigning (getf params :for-selective-assign))
          (base-indexer (base-indexer-of varray))
          (argument (render (vads-argument varray)))
          (indexer (when (functionp base-indexer)
...
;; (defmethod indexer-of ((varray vader-permute) &optional params)
;;   (if (getf params :for-selective-assign) ;; (april (with (:space array-lib-space)) "↓dsp 'hello' 'world'")
;;       (nindexer-of varray params)
;;       (generator-of varray params)))

(defmethod nindexer-of ((varray vader-permute) &optional params)
  (declare (optimize (speed 3) (safety 0)))
  (if (or (shape-of varray)
          (getf params :for-selective-assign))
      (let* ((assigning (getf params :for-selective-assign))
             (argument (render (vads-argument varray)))
...
(defmethod shape-of ((varray vacomp-each))
  (get-promised (varray-shape varray)
                (let* ((oshape (shape-of (vacmp-omega varray)))
                       ...
                       (osize (reduce #'* oshape))
                       (asize (reduce #'* ashape)))
                  ;; (print (list :os oshape ashape (vacmp-omega varray) (vacmp-alpha varray)
                  ;;              ;; (setf april::iio (vacmp-omega varray))
                  ;;              osize asize
                  ;;              ))
                  ;; (print (list :oo oshape osize ashape asize (vacmp-alpha varray)
...
(defmethod indexer-of ((varray vacomp-produce) &optional params)
  (let* ((omega (vacmp-omega varray))
         ...
         (oindexer (indexer-of omega))
         (aindexer (indexer-of alpha))
