    ...
    (if output-shape
        (if (zerop (reduce #'* output-shape))
            ...
            (let ((output (make-array (shape-of varray) :element-type (etype-of varray)))
              ...
              (let (;; (render-index
                    ;;   (if to-subrender ;(and (not to-subrender)
                    ;;                     ;   (not (subrendering-p indexed)))
                    ;;       (lambda (i)
                    ...
                    ;;       (lambda (i)
                    ;;         (declare (type fixnum i))
                    ;;         (setf (row-major-aref output i) (or (funcall indexer i) prototype)))))
                    (render-index
                      (lambda (i)
        ...
        (funcall (if (not (subrendering-p varray))
                     ...
                     ;;     (if (not (shape-of rendered))
                     ;;         rendered (enclose rendered))))
                     (lambda (item)
                       (apply #'render item params))
                     (lambda (item)
                       (let ((rendered (render item)))
                         ;; (print (list :rr rendered))
                         (if (not (arrayp rendered))
                             ;; (zerop (rank-of rendered))
...
(defmethod indexer-of ((vvector vapri-integer-progression) &optional params)
  ...
  (get-promised (varray-indexer vvector)
                (let ((origin (the (signed-byte 64) (vapip-origin vvector)))
                      (factor (the fixnum (vapip-factor vvector)))
                      (repeat (the fixnum (vapip-repeat vvector))))
                  (funcall (if (= 1 factor)
                               #'identity (lambda (fn) (lambda (item)
                                                         (declare (type fixnum item)
                                                                  (type function fn))
                                                         (* factor (funcall fn item)))))
                           (if (= 1 repeat)
                               (if (zerop origin)
                                   #'identity (if (= 1 origin)
                                                  (lambda (index)
                                                    (declare (type fixnum index))
                                                    (the (signed-byte 64) (1+ index)))
                                                  (lambda (index)
                                                    (declare (type (signed-byte 64) index))
                                                    (the (signed-byte 64) (+ origin index)))))
                               (lambda (index)
                                 (declare (type integer index))
                                 (+ origin (the fixnum (floor index repeat)))))))))
...
(defmethod indexer-of ((varray vader-reshape) &optional params)
  ...
  (get-promised
   (varray-indexer varray)
   (let* ((base-indexer (base-indexer-of varray))
          (input-size (size-of (vader-base varray)))
...
;;                                base))))
;;     (print (list :ii base))
;;     (print (list :as assigned-index))
;;     (if (and path (< path-index (1- (size-of path))))
;;         (lambda (index)
;;           (if (= index assigned-index)
;;               (let ((assigned-indexer (assign-reference varray (funcall base-indexer index)
;;                                                         path path-indexer (1+ path-index))))
;;                 (if (not (functionp assigned-indexer))
;;                     assigned-indexer (funcall assigned-indexer 0)))
;;               (funcall base-indexer index)))
;;         (lambda (index)
;;           (if (= index assigned-index)
;;               (let ((assigned-indexer (indexer-of (vapick-assign varray))))
;;                 (if (not (functionp assigned-indexer))
;;                     assigned-indexer (funcall assigned-indexer 0)))
;;               (funcall base-indexer index))))))
                      ...
                      (flet ((process-item (ix)
                               (let ((item (funcall
                                 ...
                                 (if (or scalar-fn catenate-fn)
                                     (setf (aref value (- (or window rlen) (incf valix)))
                                           item)
