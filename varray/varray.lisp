   ...
   (let* ((base (vader-base varray))
     ...
     (if (and (not is-inverse) (eq :last axis)
         ...
         (let ((shape (list (abs arg-indexer))))
           ...
           (generator-of base)
           (when (> (first shape) (first (shape-of base)))
         ...
         (let* ((base-rank (rank-of base))
           ...
           ;;       (make-array (* 2 base-rank) :element-type 'fixnum :initial-element 0)
           ...
           (if (vectorp axis)
               (loop :for x :across axis :for ix :from 0
                     :do (setf (aref pre-shape (- x iorigin))
                               (if is-inverse
                                   (let ((arg (funcall arg-indexer ix)))
                                     (when (< 0 base-rank)
                                       (setf (aref (vasec-span varray)
                                                   (+ (- x iorigin)
                                                      (* base-rank (if (< 0 arg) 0 1))))
                                             (if (> arg 0) (abs arg)
                                                 (- (aref pre-shape (- x iorigin)) (abs arg)))))
                                     (- (aref pre-shape (- x iorigin))
                                        (abs arg)))
                                   (let* ((arg (funcall arg-indexer ix))
                                          (element (abs arg)))
                                     (if (> element (aref pre-shape (- x iorigin)))
                                                  ...
                                                  (setf (aref (vasec-span varray)
                                                              (+ (- x iorigin)
                                                                 (* base-rank
                                                                    (if (>= 0 arg) 0 1))))
                                                        (if (> 0 arg)
                                                            0 (aref pre-shape (- x iorigin)))
                                                        (aref (vasec-padding varray)
                                                              (+ (- x iorigin)
                                                                 (* base-rank
                                                                    (if (>= 0 arg) 0 1))))
                                                        (- element (aref pre-shape (- x iorigin))))))
                                         (when (< 0 base-rank)
                                           (setf (aref (vasec-span varray)
                                                       (+ (- x iorigin)
                                                          (* base-rank (if (>= 0 arg) 0 1))))
                                                 (if (<= 0 arg)
                                                     element (+ arg (aref pre-shape (- x iorigin)))))))
                                     (if (<= 0 (funcall arg-indexer ix))
                                         element (+ (funcall arg-indexer ix)
                                                    (aref pre-shape (- x iorigin))))))))
               (if (eq :last axis)
                       ...
                       (loop :for a :below (first arg-shape) :for ix :from 0
                             :do (let ((arg (funcall arg-indexer a)))
                                   (setf (aref pre-shape ix)
                                         (if is-inverse
                                             (let ((element (max 0 (- (aref pre-shape ix)
                                                                      (abs arg)))))
                                               (when (< 0 base-rank)
                                                 (setf (aref (vasec-span varray)
                                                             (+ ix (* base-rank (if (< 0 arg) 0 1))))
                                                       (if (> arg 0) (abs arg)
                                                           (- (aref pre-shape ix) (abs arg)))))
                                               element)
                                             (let ((element (abs arg)))
                                               ;; (print (list :es arg pre-shape element ix (vasec-span varray)
                                               ;;              (+ ix (* base-rank
                                               ;;                       (if (>= 0 arg) 0 1)))))
                                               (if (> element (aref pre-shape ix))
                                                   (progn (setf (vasec-overtaking varray) t)
                                                          (when (< 0 base-rank)
                                                            (setf (aref (vasec-span varray)
                                                                        (+ ix (* base-rank
                                                                  ...
                                                                  (aref (vasec-padding varray)
                                                                        (+ ix (* base-rank
                                                                                 (if (>= 0 arg) 0 1))))
                                                                  (- element (aref pre-shape ix)))))
                                                   (when (< 0 base-rank)
                                                     (setf (aref (vasec-span varray)
                                                                 (+ ix (* base-rank (if (>= 0 arg) 0 1))))
                                                           (if (<= 0 arg)
                                                               element (+ arg (aref pre-shape ix))))))
                                               element)))))
                       (setf (aref pre-shape 0)
                             (let ((arg arg-indexer))
                               (if is-inverse (let ((element (max 0 (- (aref pre-shape 0) (abs arg)))))
                                                (when (< 0 base-rank)
                                                  (setf (aref (vasec-span varray)
                                                              (* base-rank (if (< 0 arg) 0 1)))
                                                        (if (> arg 0) (abs arg)
                   ...
                   (setf (aref pre-shape (- axis iorigin))
                         (let ((arg arg-indexer))
                           (if is-inverse (let ((element (max 0 (- (aref pre-shape (- axis iorigin))
                                                                   (abs arg-indexer)))))
                                            (setf (aref (vasec-span varray)
                                                        (+ (- axis iorigin)
                                                           (* base-rank (if (< 0 arg) 0 1))))
                               ...
                               (let ((element (abs arg-indexer)))
                                 (if (> element (aref pre-shape (- axis iorigin)))
                                     (progn (setf (vasec-overtaking varray) t)
                                            (when (< 0 base-rank)
                                              (setf (aref (vasec-span varray)
                                                          (+ (- axis iorigin)
                                                             (* base-rank (if (>= 0 arg) 0 1))))
                                                    (if (> 0 arg)
                                                        0 (aref pre-shape (- axis iorigin)))
                                                    (aref (vasec-padding varray)
                                                          (+ (- axis iorigin)
                                                             (* base-rank (if (>= 0 arg) 0 1))))
                                                    (- element (aref pre-shape (- axis iorigin))))))
                                     (when (< 0 base-rank)
                                       (setf (aref (vasec-span varray)
                                                   (+ (- axis iorigin)
                                                      (* base-rank (if (>= 0 arg) 0 1))))
...
(defmethod indexer-of ((varray vader-section) &optional params)
  ...
  (get-promised
   ...
   (let* ((assigning (getf params :for-selective-assign))
          ...
          (is-inverse (vads-inverse varray))
          (base-size (size-of (vader-base varray)))
          (out-dims (if is-inverse (make-array (rank-of varray) :initial-element 0)
     ...
     (let* ((indexer (indexer-section is-inverse ;; (or (shape-of (vader-base varray)) '(1))
                                      (shape-of (vader-base varray))
                                      out-dims assigning (vasec-span varray) (vasec-padding varray)))
