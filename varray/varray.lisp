  ...
  (let* ((indices)
    ...
    (lambda (index)
      (let* ((remaining index) (oindex 0) (ofix 0) (valid t)
             ...
             (choose-indexed) (iafactors iarray-factors))
        ;; (print (list :cc indices ifactors ofactors iarray-factors))
              ...
              :do (let ((afactor (if (and afactors (< ix (length afactors)))
                    ...
                    (if (numberp in) ;; handle numeric indices as for x[1;2]
                        ...
                        (if in (let ((sub-index 0))
                                 ...
                                 ;; (print (list :si sub-index
                                 ;;              (funcall (indexer-of in) sub-index)))
        ...
        (if valid (print (list :oo oindex valid sub-index)))
        (if (numberp oindex)
            ...
            (let ((index-shape (first (shape-of oindex))))
              ...
              (setf (vads-subrendering varray) t)
              (if (and (= index-shape (length (shape-of (vader-base varray))))
                       (numberp (funcall base-indexer 0)))
                  ...
                  ;; (as for (6⍴('JAN' 1)('FEB' 2)('MAR' 3))[(2 1)(1 2)])
                  ...
                  (let* ((meta-indexer (indexer-of oindex))
                    ...
                    ;; (print (list :rr (render (make-instance 'vader-select
                    ;;                                         :base (vader-base varray)
                    ;;                                         :index-origin (vads-io varray)
                    ;;                                         :argument argument))
                    ;;              (rest (coerce (render oindex) 'list))))
...
(make-instance
 'vader-select :base (if (eq :nested choose-indexed)
                         (vader-base varray)
                         (if (numberp oindex)
                             (funcall base-indexer oindex)
                             (let* ((oindexer (indexer-of oindex))
                                    (oindexed (if (not (functionp oindexer))
                                                  oindexer (funcall oindexer 0)))
                                    (indices (if (vectorp oindexed)
                                                 (coerce oindexed 'list)
                                                 (coerce oindex 'list))))
                               ;; TODO: wrap this in a disclose object once that's ready
                               (disclose (render
                                          (make-instance
                                           'vader-select :base (vader-base varray)
                                           :index-origin (vads-io varray)
                                           :argument indices))))))
 :index-origin (vads-io varray)
 :argument (if (eq :nested choose-indexed)
               (coerce (render oindex) 'list)
               (rest (coerce (render choose-indexed)
                             'list))))
...
;;                                       (if (= ix (1- (length ifactors)))
;;                                           (incf oindex)))
;;                                   ;; (let ((this-shape (shape-of indexed)))
;;                                   ;;   ;; (print (list :ba base-rank this-shape))
;;                                   ;;   (if (and this-shape (= base-rank (first this-shape)))
;;                                   (if in
;;                                       ;; (let ((this-shape (shape-of in))
;;                                       ;;       (this-indexer (indexer-of in)))
;;                                       ;;   (if (not (numberp)))
...
;;                   :do (if (numberp in)
;;                           (incf oindex (* ifactor (- in (vads-io varray))))
;;                           (if (or (not in) (vectorp in))
;;                               (multiple-value-bind (index remainder) (floor remaining (aref ofactors ofix))
;;                                 (let ((indexed (if in (funcall (indexer-of in) index))))
;;                                   (if (or (not in) (numberp indexed))
;;                                       (incf oindex (* ifactor (if (not in)
;;                                                                   index (- indexed (vads-io varray)))))
;;                                       (let ((this-shape (shape-of indexed)))
;;                                         ;; (print (list :ba base-rank this-shape))
;;                                         (if (and this-shape (= base-rank (first this-shape)))
;;                                             ;; if the shape of the index is the same as the base array,
;;                                             ;; use it to index the argument for reach indexing
;;                                             ;; as with (6⍴('JAN' 1)('FEB' 2)('MAR' 3))[(2 1)(1 2)]
;;                                             (setf oindex indexed
;;                                                   choose-indexed (if (numberp indexed)
;;                                                                      :nested indexed)
;;                                                   (vads-subrendering varray) t)
;;                                             (let ((sub-indexer (indexer-of indexed)))
;;                                               ;; (print (list :ind indexed))
;;                                               (setf oindex (- (funcall sub-indexer 0)
;;                                                               (vads-io varray))
;;                                                     choose-indexed indexed
;;                                                     (vads-subrendering varray) t)))))
;;                                   ;; if choose indexing is in use, set this object to subrender
;;                                   (incf ofix)
;;                                   (setf remaining remainder)))
...
;;             ;;                 (incf ofix)
;;             ;;                 (setf remaining remainder))))
;;             ;; (print (list :oin oindex choose-indexed))
;;             (if (not choose-indexed)
;;                 (let ((indexed (if (not (functionp base-indexer))
;;                                    base-indexer (funcall base-indexer oindex))))
;;                   (if (not (shape-of varray))
;;                       (setf (vads-subrendering varray) t))
