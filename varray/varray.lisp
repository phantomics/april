          ...
          (loop :for item :across (vader-base varray)
                :when (< 0 (size-of item))
                  :do (let ((this-indexer (indexer-of item)))
                        (if (not pre-prototype)
                            (setf pre-prototype (if (not (functionp this-indexer))
                                                    this-indexer (funcall this-indexer 0)))
                            (setf pre-prototype (apply-scalar (vaop-function varray)
                                                              pre-prototype
                                                              (if (not (functionp this-indexer))
                                                                  this-indexer (funcall this-indexer 0)))))
                            
                          ;; (let ((this-proto (prototype-of item)))
                          ;; (setf prototype (prototype-of (apply-scalar (vaop-function varray)
                          ;;                                             prototype (prototype-of item))))
                            ;; (if (and (not (shape-of item))
                            ;;          (shape-of this-proto))
                            ;;     (setf prototype (prototype-of (funcall (vaop-function varray)
                            ;;                                            this-proto prototype)))
                            ;;     (if (< (length (shape-of item))
                            ;;            (length (shape-of this-proto)))
                            ;;         (setf prototype this-proto)))
                            ))
          (prototype-of pre-prototype))))
          ...
          (loop :for a :across (vader-base varray) :for ix :from 0
                :do (let* ((shape (shape-of a))
                      ...
                      (if (or (arrayp item) (varrayp item))
                          (progn (if (varrayp item) (setf (vads-subrendering varray) t))
                                 (push item subarrays))
