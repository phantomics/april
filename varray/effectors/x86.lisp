...
;; (defun rotate (array amount &optional (axis 0))
;;   (petalisp:with-lazy-arrays (array)
;;     (let* ((shape (petalisp:lazy-array-shape array))
...
;;          (petalisp:lazy-reshape array lo transformation))))))
...
(defmethod plex-of :around ((varray vapri-apro-vector) &optional params)
  (when (and (zerop (vapip-origin varray))
             ...
             (= 1 (vapip-repeat varray)))
    (petalisp:lazy-index-components (~ (vapip-number varray)))))
