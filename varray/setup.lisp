...
;;                           (type (simple-array (unsigned-byte 8) (+rank-plus+))
;;                                 indices)) ;; TODO: fix hardcoded type
;;                  (let ((os (the (unsigned-byte 8)
;;                                 (max 0 (- (/ +eindex-width+ +cindex-width+) +rank-plus+)))))
;;                    (the (function ((unsigned-byte +eindex-width+)) (unsigned-byte +eindex-width+))
;;                         (lambda (i)
;;                           (declare (type (unsigned-byte +eindex-width+) i))
;;                           (let ((iindex (the (unsigned-byte +eindex-width+) 0)))
;;                             (loop :for a :of-type (unsigned-byte +cindex-width+) :across indices
;;                                   :for n :of-type (unsigned-byte 8) ; :from 0 ; os
;;                                   ;; :from (max 0 (- +rank-plus+ os)) :downto 0
;;                                   := (max 0 (- +rank-plus+ os)) :then (1- n)
;;                                   :do (setf iindex (dpb (ldb (byte +cindex-width+ (* n +cindex-width+)) i)
;;                                                         (byte +cindex-width+
;;                                                               (the (unsigned-byte 8)
;;                                                                    (max 0 (* (- +rank-plus+ a os)
;;                                                                              +cindex-width+))))
;;                                                         iindex))
;;                                       ;; (print (list :abc (- n os) (- (- +rank-plus+ a) os)))
;;                                   )
;;                             ;; (print (format nil "#x~8,'0X" i))
;;                             ;; (print (format nil "#x~8,'0X~%" iindex))
;;                             iindex)))))))
