  ...
  (let* ((omega (vacmp-omega varray))
    ...
    (if (or (> 2 osize) (vacred-unitary varray))
        (if (zerop irank)
            ...
            (if (not (or (typep omega 'vacomp-reduce)
                         ;; (not (shape-of (vacmp-omega varray)))
                         (and (arrayp omega)
            ...
            (if (zerop osize)
                ...
                (case (getf params :base-format)
                  (:encoded)
                  (:linear)
                  (t ;; (if t ; (typep omega 'vacomp-reduce)
                     ;;     (generator-of omega)
                     ;;      omega
                     ;;     ;; (if (= 1 irank)
                     ;;     ;;     (make-instance 'vader-enclose :base (funcall (generator-of omega) 0))
                     ;;     ;;     (make-instance 'vader-pare :base omega :index-origin (vads-io varray)
                     ;;     ;;                                :axis (if (= 1 irank)
                     ;;     ;;                                          nil (vads-axis varray))))
                     ;;      )
                   (if (= 1 irank)
                       (let* ((ogen (generator-of omega))
                         ...
                         (if (and (/= 1 (size-of omega))
                                  (= 1 (size-of oitem)))
                             (if (print (not (or (arrayp oitem) (varrayp oitem))))
                                 oitem (generator-of (make-instance 'vader-enclose :base oitem)))
                             ogen
                             ;; (if (or (arrayp oitem) (varrayp oitem))
