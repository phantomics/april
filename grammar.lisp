...
(defun process-value (this-item properties process idiom space)
  ...
  (cond ((and (listp this-item)
        ...
        ((and (listp this-item)
         ...
         (let ((nspath (format-nspath (if (not (symbol-value (intern "*NS-POINT*" space)))
                                          (rest this-item)
                                          (append (symbol-value (intern "*NS-POINT*" space))
                                                  (rest this-item))))))
           (if (or (not nspath) (not (fboundp (intern nspath space)))
                   (getf properties :symbol-overriding))
               (values (if (symbol-value (intern "*NS-POINT*" space))
                           (cons 'nspath (append (symbol-value (intern "*NS-POINT*" space))
        ...
        ((and (symbolp this-item)
         ...
         (values (if (not (member (intern (string-upcase this-item) *package-name-string*)
                                  (rest (assoc :function (idiom-symbols idiom)))))
                     (if (or (not (symbol-value (intern "*NS-POINT*" space)))
                         ...
                         this-item (cons 'nspath (append (symbol-value (intern "*NS-POINT*" space))
...
(defun process-function (this-item properties process idiom space)
  ...
  (if (listp this-item)
      ...
      (if (and (symbolp this-item)
               (not (getf properties :glyph)))
          (cond ((and (symbol-value (intern "*NS-POINT*" space))
                      (fboundp (intern (format-nspath (append (symbol-value (intern "*NS-POINT*" space))
                                                              (list this-item)))
                                       space)))
                 (values (intern (format-nspath (append (symbol-value (intern "*NS-POINT*" space))
                                                        (list (intern (string this-item)
                                                                      "KEYWORD")))))
    ...
    (if (and (member :left-operand-value (getf function-props :type))
        ...
        (if (and function-form is-function)
            (values (if (or (not axes) (of-lexicons idiom function-form :functions))
                        ...
                        (if (not (and (symbolp function-form)
                                      (or (member :at-path (getf function-props :type))
                                          (is-workspace-function function-form))))
                           ...
                           (if (and (listp symbol) (eql 'nspath (first symbol)))
                               (let ((path-symbol (intern (format-nspath (rest symbol)) space)))
                                 ...
                                 `(setf ,(macroexpand symbol) :function
                                        (symbol-function ',path-symbol) ,precedent))
  ...
  (if is-function (let ((fn-content (if (and (symbolp fn-element) (eql '∇ fn-element))
                                        ...
                                        (if (or (functionp fn-element)
                                            ...
                                            fn-element (if (characterp fn-element)
                                                           ...
                                                           (or (of-lexicons idiom fn-element :functions)
                                                               fn-element))))))
                    ;; (print (list :pro properties (symbol-value (intern "*NS-POINT*" space))
                    ;;              precedent))
                    (if (and (listp fn-content) (eql 'function (first fn-content))
                        ...
                        (setf (symbol-value (intern "*NS-POINT*" space))
                              (if (listp precedent)
                                  (if (eql 'nspath (first precedent))
                                      (cons (intern (string (second (second precedent))) space)
                                            (loop :for item :in (cddr precedent) :collect (intern (string item)
                                                                                                  "KEYWORD")))
                                      (if (not (string= "_" (string (second precedent))))
                                          (list (intern (string (second precedent)) space)))))))
