        ...
        (if path-val
            ...
            (if (or (symbolp symbol)
                ...
                input-sym (cons 'nspath (cons (if (member (second symbol) '(⍵ ⍺ ⍹ ⍶))
                                                  (second symbol)
                                                  (list 'inwsd (intern (string (if (listp (second symbol))
                                                                                   (cadadr symbol)
                                                                                   (second symbol)))
                                                                       *package-name-string*)))
  ...
  (cond ((eq :empty-array this-item)
        ...
        ((and (symbolp this-item)
              (or (member this-item '(⍵ ⍺ ⍹ ⍶) :test #'eql)
                  ...
                  (or (of-meta-hierarchy2 (rest (getf (getf properties :special) :closure-meta))
                                          :var-syms this-item)
              ...
              (or (not (is-workspace-operator this-item))
                  (of-meta-hierarchy2 (rest (getf (getf properties :special)
                                                                   :closure-meta))
                                      :var-syms this-item)
                  (getf properties :symbol-overriding))
        ...
        (if (and (symbolp this-item)
            ...
            (cond ((and current-path (fboundp (intern (format-nspath (append current-path (list this-item)))
                  ...
                  ((and (is-workspace-function this-item)
                        ...
                        (not (of-meta-hierarchy2 (rest (getf (getf properties :special) :closure-meta))
                                                 :var-syms this-item))
                        (not (or (of-meta-hierarchy2 (rest (getf (getf properties :special) :closure-meta))
                                                     :fn-syms this-item)
                                 (of-meta-hierarchy2 (rest (getf (getf properties :special) :closure-meta))
                                                     :lop-syms this-item)
                                 (of-meta-hierarchy2 (rest (getf (getf properties :special) :closure-meta))
  ...
  (if (listp this-item)
      ...
      (if (symbolp this-item)
          ...
          (let* ((symbol-string (string this-item))
            ...
            (if (and lop-string
                     ;; make sure it's not defined locally as a variable
                     (not (of-meta-hierarchy2 (rest (getf (getf properties :special)
                                                          :closure-meta))
                                              :var-syms this-item))
                     (or (and (fboundp (intern lop-string space))
                              ...
                              (eq :lateral (getf (rest (symbol-value (intern lop-string space)))
                                                 :valence)))
                         (of-meta-hierarchy2 closure-meta :lop-syms this-item)))
                (if (not (of-meta-hierarchy2 closure-meta :lop-syms this-item))
                ...
                (if (and pop-string
                         (or (and (fboundp (intern pop-string space))
                                  ...
                                  (eq :pivotal (getf (rest (symbol-value (intern pop-string space)))
                                                     :valence)))
                             (of-meta-hierarchy2 closure-meta :pop-syms this-item)))
                    (if (not (of-meta-hierarchy2 closure-meta :pop-syms this-item))
  ...
  (cond ((and (first tokens) (listp (first tokens)) ;; handle enclosed functions like (,∘×)
         ...
         (if found-function (values found-function tokens)
             ...
             (multiple-value-bind (sub-function remaining)
               ...
               (if sub-function ;; catch errors like (2+) 5
                   (if remaining (error "Value to left of function statement.")
        ...
        ((not found-function) ;; this is the beginning of the function composition
         (let* ((exp-operator (build-operator (list (first tokens))
                ...
                (value-operand (or (of-meta-hierarchy2 (rest (getf (getf params :special)
                                                                   :closure-meta))
