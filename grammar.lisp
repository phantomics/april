...
(defun process-value (this-item properties process idiom space)
  ...
  (cond ((and (listp this-item)
        ...
        ((and (listp this-item)
         ...
         (if t; (not (functionp (follow-path space (rest this-item))))
  ...
  (if (listp this-item)
      ...
      (if (or (eq :fn (first this-item))
          ...
          (if (not (getf properties :glyph))
              (if (and (eq :pt (first this-item))
                       ;; (if (symbol-value (intern "*NS-POINT*" space))
                       ;;     (functionp (follow-path space (mapcar (lambda (item) (intern (string item)
                       ;;                                                                  "KEYWORD"))
                       ;;                                           (rest this-item))
                       ;;                             (follow-path space (rest this-item))))
                       ;;     (functionp (follow-path space (rest this-item))))
                       )
                           ...
                           (if (eql '⍺ symbol)
                               ...
                               (if (of-lexicons idiom precedent :functions)
                                   `(setf ,(if (and (listp symbol) (eql 'nspath (first symbol)))
                                               ...
                                               (if at-top-level `(symbol-function '(inws ,symbol))
                                                          `(inws ,symbol)))
