      ...
      `(progn ,@(if (not extension)
	      ...
	      ,@(if (not extension)
			...
			(cond ((and ,options (listp ,options)
		      ...
		      (defmacro ,(intern (concatenate 'string "WITH-" (string-upcase symbol) "-CONTEXT")
					 (package-name *package*))
			  (,options &rest ,body)
			(labels (;; (merge-options (,origin ,target)
				 ;;   (if (not ,origin)
				 ;;       ,target (merge-options (cddr ,origin)
				 ;; 			      (cond ((or (eql :in (first ,origin))
				 ;; 					 (eql :out (first ,origin)))
				 ;; 				     (setf (getf ,target (first ,origin))
				 ;; 					   (loop :for ,item
				 ;; 					      :in (getf ,target (first ,origin))
				 ;; 					      :collect (if (assoc (first ,item)
				 ;; 								  (second ,origin))
				 ;; 							   (assoc (first ,item)
				 ;; 								  (second ,origin))
				 ;; 							   ,item)))
				 ;; 				     ,target)
				 ;; 				    (t (setf (getf ,target (second ,origin))
				 ;; 					     (first ,origin))
				 ;; 				       ,target)))))
				 (,process (,form)
				   (loop :for ,item :in ,form
				      :collect (if (and (listp ,item)
							(eql ',(intern (string-upcase symbol)
								       (package-name *package*))
							     (first ,item)))
						   (list (first ,item)
							 ;; (if (third ,item)
							 ;;     (cons (caadr ,item)
							 ;; 	   (list (merge-options (cadadr ,item)
							 ;; 				(second ,options))))
							 ;;     ,options)
							 ,options
							 (first (last ,item)))
