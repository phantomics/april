  ...
  (if (not exp)
	...
	(if (not precedent)
		...
		(vex-exp idiom meta from-value
			 (apply (first (cadar operation)) ;;(first (cadar (last operation)))
				`(,meta ,(if (eq :axes (caadr operation))
					     (list (cadadr operation)))
					,@(if right-value (list right-value))
					,precedent)))))))))

;; ((9 (:FN #\APL_FUNCTIONAL_SYMBOL_IOTA) (:FN #\APL_FUNCTIONAL_SYMBOL_RHO) 8 7
;;     (:AXES #((1))) (:FN #\APL_FUNCTIONAL_SYMBOL_CIRCLE_STILE) 2))
