...
(defun =vex-tree (idiom meta &optional output)
  (labels ((?blank-character ()
	   ...
	   (?newline-character ()
	     (?satisfies (of-utilities idiom :match-newline-character)))
	   ...
	   (=vex-axes () ;; handle axes, separated by semicolons
		 ...
		 (=list (=subseq (%some (?satisfies (lambda (char) (not (char= char #\;))))))
			(=subseq (%any (?satisfies (funcall (lambda ()
							      (let ((index 0))
								(lambda (char)
								  (incf index 1)
								  (and (not (< 1 index))
...
(defun vex-exp (idiom meta exp &optional precedent)
  ...
  (if (not exp)
	...
	(if (not precedent)
		...
		(vex-exp idiom meta from-value (funcall (caar (last operation))
