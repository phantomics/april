...
(defun merge-options (source &optional target)
  (loop :for section :in target
     :collect (let ((items (rest section))
		    (pair-index 0))
		(loop :for ssection :in source :when (eq (first section) (first ssection))
		   :do (loop :for (item-name item-value) :on (rest ssection) :while item-value
			  :do (if (evenp pair-index)
				  (setf (getf items item-name) item-value))
			  (incf pair-index)))
		(cons (first section)
		      items))))
...
(defmacro vex-idiom-spec (symbol extension &rest subspecs)
  ...
  (macrolet ((of-subspec (symbol-string)
    ...
    (let* ((symbol-string (string-upcase symbol))
	   ...
	   (process (gensym)) (form (gensym)) (item (gensym)) (origin (gensym)) (target (gensym)))
			...
			(cons ',(intern symbol-string (package-name *package*))
			      (append (if (second ,options)
					  (list (cons (caar ,options)
						      (merge-options `((:state :print-output t))
								     (cdar ,options))))
					  `((set (:state :print-output t))))
				      (last ,options))))
			...
			(labels ((,process (,form)
				   (loop :for ,item :in ,form
				      :collect (if (and (listp ,item)
							(eql ',(intern symbol-string (package-name *package*))
							     (first ,item)))
						   (list (first ,item)
							 (if (third ,item)
							     (cons (caadr ,item)
							 	   (list (merge-options (cdadr ,item)
							 				(rest ,options))))
							     ,options)
							 (first (last ,item)))
						   (if (listp ,item)
