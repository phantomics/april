	       ...
	       (if specs (let ((this-spec (cdar specs)))
			   (process-gentests (rest specs)
					     (append output `((princ ,(getf this-spec :title))
								  ...
								  :test #'equalp)))))
		   output)))
      (process-pairs 'fn-specs :functions (rest (assoc (intern "FUNCTIONS" (package-name *package*))
						       subspecs)))
      (let* ((operator-specs (process-pairs 'op-specs :operators
					    (rest (assoc (intern "OPERATORS" (package-name *package*))
							 subspecs))))
	     (function-specs (process-pairs 'fn-specs :functions
