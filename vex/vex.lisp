...
;; 		      ,precedent ,right-value)
;; 	    `(funcall ,(cadar operation)
;; 		      ,precedent)))
;; ((eq :fndef (caar operation))
;; 	(if right-value
;; 	    (funcall (cadar operation)
;; 		     meta nil precedent right-value)
;; 	    (funcall (cadar operation)
;; 		     meta nil precedent)))
;; ((eq :fn (caar operation))
;; 	(print (list :oo operation))
;; 	(cond ((and (< 1 (length operation))
;; 		    (eq :op (caar (last operation)))
;; 		    (eq :right (caadar (last operation))))
;; 	       (print (list "EBEB" (cadar operation)))
;; 	       (apply (second (cadar (last operation)))
;; 		      `(,meta nil ,(cadar operation)
;; 			      ,@(if right-value (list right-value))
;; 			      ,precedent)))
;; 	      (t (apply (first (cadar (last operation)))
;; 			`(,meta ,nil ,@(if right-value (list right-value))
;; 				,precedent)))))
;; )
;; (t (if right-value
;; 	    (funcall (first (cadar (last operation)))
