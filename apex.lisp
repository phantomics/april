...
(defclass idiom ()
  ((name :accessor idiom-name
	 :initarg :name)
   (exp-matcher :accessor spec-exp-matcher
		:initform ""
		:initarg :exp-matcher)
   ...
   (discrete-functions :accessor idiom-discrete-functions
	       :initform nil
		       :initarg :discrete-functions)))
...
(defmacro vex-spec (symbol &rest subspecs)
  ...
  (labels ((process-function-definition (is-dyadic function-spec)
	     (if (and (listp (first function-spec))
	   ...
	   (assign-discrete-functions (specs)
	     ...
	     (let ((spec (second specs)))
		   ...
		   (list (process-function-definition nil (last (first (last spec)))))
		   (if (eql 'dyadic (first spec))
		       (list (process-function-definition t (last (first (last spec)))))
		       (list (if (listp (second spec))
				 (process-function-definition nil (last (second spec)))
				 (if (eq :symmetric-scalar (second spec))
				     (list 'function (third spec))
				     (cons 'function (last spec))))
			     (if (listp (second spec))
				 (process-function-definition t (last (third spec)))
				 (cons 'function (last spec))))))))
...
(defun apply-scalar-function (function alpha omega)
  (let* ((alpha (if (arrayp alpha)
	 ...
	 (singleton-alpha (or (not (arrayp alpha))
			      (is-singleton alpha)))
	 (singleton-omega (or (not (arrayp omega))
			      (is-singleton omega))))
	...
	(if singleton-alpha
	    (if singleton-omega
		(aops:each function alpha omega)
		(aops:each function (scale-array alpha omega)
			   omega))
	    (if singleton-omega
		(aops:each function alpha (scale-array omega alpha))
		(let ((mapped-output (aops:each (lambda (1st 2nd) (apply-scalar-function function 1st 2nd))
						alpha omega)))
		  (if (loop for index from 0 to (1- (length mapped-output))
			 always (aref mapped-output index))
		      (aops:each (lambda (item) (if (and (= 1 (length (dims item)))
							 (= 1 (length item)))
						    (aref item 0)
						    item))
				 mapped-output))))))))
...
(defun multidim-slice (array dimensions options)
  (if (= 1 (length dimensions))
      (apply #'aops:stack
	     (append (list 0 (aops:partition array (if (eq :negative (getf options :polarity))
						       (first dimensions)
						       0)
					     (if (eq :negative (getf options :polarity))
						 (first (dims array))
						 (if (< (first (dims array))
							(first dimensions))
						     (first (dims array))
						     (first dimensions)))))
		     (if (and (eq :positive (getf options :polarity))
			      (< (first (dims array))
				 (first dimensions)))
			 (list (make-array (list (- (first dimensions)
						    (first (dims array))))
					   :initial-element (getf options :fill-with))))))
      (aops:combine (apply #'aops:stack
			   (append (list 0 (aops:each (lambda (a) (multidim-slice a (rest dimensions) options))
						      (subseq (print (aops:split array 1))
							      (if (eq :negative (getf options :polarity))
								  (first dimensions)
								  0)
							      (if (eq :negative (getf options :polarity))
								  (first (dims array))
								  (if (< (first (print (dims array)))
									 (print (first dimensions)))
								      (first (dims array))
								      (first dimensions))))))
				   (if (and (eq :positive (getf options :polarity))
					    (< (first (dims array))
...
(vex-spec
 ...
 (operators / (right (lambda (axes functions operand)
			 ...
			 (if (= 1 (rank operand))
			     (reduce (caar functions)
				     operand)
			     (aops:margin (lambda (sub-array) (reduce (caar functions)
								  sub-array))
			 ...
			 (if (= 1 (rank operand))
			     ...
			     (aops:margin (lambda (sub-array) (reduce (caar functions)
					  ...
					  new-array (if axes (1- (aref (first axes) 0))
							0))))))
	   \\ (right (lambda (axes functions operand)
  		       (apply-marginal (make-back-scanner (caar functions))
	    ...
	    ⍀ (right (lambda (axes functions operand)
		       (apply-marginal (make-back-scanner (caar functions))
				       operand (if axes (1- (aref (first axes) 0)))
				       0)))
  	   \. (center (lambda (axes functions operand right-operand &optional ebeb)
			(declare (ignore axes))
			(if (eq :outer-product-designator (second functions))
			    (aops:outer (cadar functions)
				...
				(aops:each (lambda (sub-vector)
					   ...
					   (aops:outer (lambda (arg1 arg2)
							 (if (or (arrayp arg1)
								 (arrayp arg2))
							     (apply-scalar-function (cadar functions)
										    arg1 arg2)
							     (funcall (cadar functions)
								      arg1 arg2)))
						       (aops:split (aops:permute (alexandria:iota
										  (rank operand))
										 operand)
								   1)
						       (aops:split (aops:permute (reverse (alexandria:iota
											   (rank right-operand)))
										 right-operand)
								   1)))))))
 ...
 (functions ← (dyadic (args :sym :any set))
	    ...
	    + (ambivalent :symmetric-scalar +)
     	   ...
     	   \| (ambivalent :asymmetric-scalar abs (lambda (alpha omega) (mod omega alpha)))
     	    ...
     	    ? (ambivalent (args :scalar (lambda (omega) (1+ (random omega))))
	   ...
	   \~ (monadic (args :scalar (boolean-op (lambda (omega)
	    ...
	    ≢ (ambivalent (args :any (lambda (omega) (first (dims omega))))
			  ;; NOTE: equalp is not exactly right for this function
	    ...
	    ⍳ (ambivalent (args :one (lambda (omega)
     				       (make-array (list omega)
     						   :initial-contents (mapcar #'1+ (alexandria:iota omega)))))
     			  (args :any :any (lambda (alpha omega)
     					    (if (not (vectorp alpha))
     						(error "Rank error.")
     						(let* ((to-find (remove-duplicates alpha :from-end t))
     						       (maximum (1+ (length to-find)))
     						       (results (alexandria:copy-array omega)))
     						  (dotimes (index (array-total-size results))
     						    (setf (row-major-aref results index)
     							  (let ((found (position (row-major-aref results index)
     										 to-find)))
     							    (if found (1+ found)
     								maximum))))
     						  results)))))
	   ...
	   \, (ambivalent (args :any (lambda (omega)
	    ...
	    ⍉ (ambivalent (args :any (lambda (omega)
				       (aops:permute (reverse (alexandria:iota (rank omega)))
						     omega)))
			  (args :any :any (lambda (alpha omega)
					    (aops:permute (mapcar #'1- (array-to-list alpha))
							  omega))))
            ;; dyadic functions of ⍒⍋ nyi - and these don't work right for multidim. arrays
  ...
  (labels ((?blank-character ()
	   ...
	   (?token-character ()
	     (%or (?satisfies 'alphanumericp)
		  ...
		  (?satisfies (lambda (c) (member c (list #\macron #\. #\⍺ #\⍵ #\⍬))))))
	   ...
	   (handle-function (input-string)
		 ...
		 (if alpha `(aref (apply-scalar-function ,formatted-function ,(macroexpand alpha)
							 ,(macroexpand omega))
				  0)
		     `(aref (aops:each ,formatted-function ,(macroexpand omega))
			    0)))))
...
(defun grade (array compare-by)
  (let* ((total-size (array-total-size array))
	 (graded-array (make-array (dims array)))
	 (displaced-array (make-array (list total-size) :displaced-to array))
	 (displaced-graded-array (make-array (list total-size) :displaced-to graded-array)))
    (loop for index from 1 to total-size do (setf (aref displaced-graded-array (1- index))
						  index))
    (stable-sort displaced-graded-array (lambda (a b) (funcall compare-by (aref displaced-array (1- a))
							       (aref displaced-array (1- b)))))
    graded-array))
