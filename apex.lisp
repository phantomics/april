	       ...
	       (labels ((for-tests (tests &optional output)
			  (if tests
			      (for-tests (rest tests)
					 (append (list (cond ((eql 'is (caar tests))
		 ...
		 (if specs
		     (process-tests (rest specs)
					...
					(append output (list `(princ ,heading))
						(for-tests (rest (assoc (intern "TESTS"
										(package-name *package*))
									(rest (first specs)))))
...
(defun apply-scalar-function (function alpha omega)
  (let* ((alpha-scalar? (not (arrayp alpha)))
    ...
    (cond ((and alpha-scalar? omega-scalar?)
	  ...
	  (t ;; (aops:each (lambda (alpha omega) (apply-scalar-function function alpha omega))
	     ;; 		(if alpha-unitary?
	     ;; 		    (make-array (dims omega) :initial-element (aref alpha 0))
	     ;; 		    (if alpha-scalar?
	     ;; 			(make-array (dims omega) :initial-element alpha)
	     ;; 			alpha))
	     ;; 		(if omega-unitary?
	     ;; 		    (make-array (dims alpha) :initial-element (aref alpha 0))
	     ;; 		    (if omega-scalar?
	     ;; 			(make-array (dims alpha) :initial-element omega)
	     ;; 			omega)))
	     (labels ((scan-over (element)
		       (if (arrayp element)
			   (aops:each #'scan-over element)
			   (apply function (if alpha-unitary? (list (if (vectorp alpha)
									(aref alpha 0)
									alpha)
								    element)
					       (list element (if (vectorp omega)
								 (aref omega 0)
								 omega)))))))
 ...
 (functions (← (has :title "Assign")
		...
		(monadic (args :scalar (boolean-op (lambda (omega)
	       ...
	       (dyadic (args :scalar (boolean-op <=))))
     	    (= (has :title "Equal")
	       (dyadic (args :scalar (boolean-op =)))
	       (tests (is "2=1 2 3 4 5" #(0 1 0 0 0))))
